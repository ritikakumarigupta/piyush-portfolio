...
import { Mail, Trash2, CheckCircle2, Clock, Send, MessageSquare } from 'lucide-react';
...
export default function AdminInquiriesTab({
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b border-neutral-800/80 pb-4">
        <div>
          ...
          <p className="text-xs text-neutral-400 mt-1">
            Realtime inquiries submitted through the portfolio contact form.
          </p>
        </div>
      </div>
      ...
      {inquiries.length === 0 ? (
        <div className="bg-neutral-900/40 border border-neutral-800/80 rounded-3xl p-16 text-center backdrop-blur-xl">
          ...
          <p className="text-xs text-neutral-400 max-w-sm mx-auto">
            Client proposals and project messages submitted on your portfolio will appear here instantly.
          </p>
        </div>
      ) : (
            ...
            return (
              <div
                ...
                className={`p-6 rounded-2xl border transition-all duration-300 flex flex-col md:flex-row items-start justify-between gap-6 ${
                  isNew
                    ? 'bg-neutral-900/80 border-blue-500/30 shadow-[0_0_25px_rgba(59,130,246,0.08)]'
                  ...
                  <div className="flex flex-wrap items-center gap-2 text-xs">
                    {inq.projectType && (
                      <span className="px-3 py-1 rounded-lg bg-neutral-800/80 text-neutral-300 border border-neutral-700/60 font-medium">
                        Project: <span className="text-white font-semibold">{inq.projectType}</span>
                      </span>
                    )}
                    {inq.budgetRange && (
                      <span className="px-3 py-1 rounded-lg bg-neutral-800/80 text-neutral-300 border border-neutral-700/60 font-medium">
